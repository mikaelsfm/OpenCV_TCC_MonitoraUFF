...
int main(int argc, char** argv) {
    ...
    try {
        ...
        cout << "[INFO] Modelo YOLOv8N carregado na GPU (CUDA)." << endl;
    ...
    while (true) {
        ...
        for (auto& d : detections) {
            ...
            size_t hashValue = std::hash<string>{}(d.class_name);
            Scalar color(
                (hashValue & 0xFF), 
                ((hashValue >> 8) & 0xFF),
                ((hashValue >> 16) & 0xFF)
            ...
            textX = max(d.box.x, min(textX, d.box.x + d.box.width - textSize.width));
            textY = max(d.box.y + textSize.height, min(textY, d.box.y + d.box.height));
            ...
            putText(frame, label, Point(textX, textY),
                    FONT_HERSHEY_SIMPLEX, fontScale,
                    Scalar(0, 0, 0), thickness);
        }


        Mat displayFrame;
        double scale = 0.7;  // ajuste: 0.5, 0.7, 0.8...
        
        resize(frame, displayFrame, Size(), scale, scale);
        imshow("YOLOv8N Real-time", displayFrame);
